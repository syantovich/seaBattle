;(function() {
	...
	*/
	...
	// игровое поле игрока
	const myArea_field = getElement('myArea');
	// игровое поле компьютера
	const pcArea = getElement('pcArea');
	...
	class Field {
		...
		randomLocationShips() {
			for (let type in Field.SHIP_DATA) {
				...
				let kolpalub = Field.SHIP_DATA[type][1];
				// прокручиваем кол-во кораблей
				for (let i = 0; i < count; i++) {
					// получаем координаты первой палубы и направление расположения палуб (корабля)
					let options = this.getCoordsDecks(kolpalub);
					// кол-во палуб
					options.kolpalub = kolpalub;
					// имя корабля, понадобится в дальнейшем дл	я его идентификации
		...
		getCoordsDecks(kolpalub) {
			// получаем коэффициенты определяющие направление расположения корабля
			// ky == 0 и kx == 1 — корабль расположен горизонтально,
			// ky == 1 и kx == 0 - вертикально.
			let ky = Field.getRandom(1), kx = (ky == 0) ? 1 : 0,
				x, y;

			// в зависимости от направления расположения, генерируем
			// начальные координаты
			if (ky == 0) {
				x = Field.getRandom(9); y = Field.getRandom(10 - kolpalub);
			} else {
				x = Field.getRandom(10 - kolpalub); y = Field.getRandom(9);
			}

			const obj = {x, y, ky, kx}
			// проверяем валидность координат всех палуб корабля
			const result = this.checkLocationShip(obj, kolpalub);
			// если координаты невалидны, снова запускаем функцию
			if (!result) return this.getCoordsDecks(kolpalub);
			return obj;
		}

		checkLocationShip(obj, kolpalub) {
			let { x, y, ky, kx, fromX, toX, fromY, toY } = obj;
			...
			else if (x + ky * kolpalub < 9 && ky == 1) toX = x + ky * kolpalub + 1;
			// корабль расположен горизонтально вдоль нижней границы игрового поля
			else if (x == 9 && ky == 0) toX = x + 1;
			// корабль расположен горизонтально где-то по середине игрового поля
			else if (x < 9 && ky == 0) toX = x + 2;

			// формируем индексы начала и конца выборки по столбцам
			// принцип такой же, как и для строк
			fromY = (y == 0) ? y : y - 1;
			if (y + kx * kolpalub == 10 && kx == 1) toY = y + kx * kolpalub;
			else if (y + kx * kolpalub < 10 && kx == 1) toY = y + kx * kolpalub + 1;
			else if (y == 9 && kx == 0) toY = y + 1;
			else if (y < 9 && kx == 0) toY = y + 2;
			...
			if (this.matrix.slice(fromX, toX)
				...
				.length > 0) {console.log(this.matrix);
					return false;}
	...
	class Ships {
		constructor(self, { x, y, ky, kx, kolpalub, shipname }) {
		...
		static showShip(self, shipname, x, y, ky) {
			...
			div.style.cssText = `left:${y * Field.sizeShip}px; top:${x * Field.sizeShip}px;`;
			self.field.appendChild(div);
		}

		createShip() {
			let { player, field, shipname, kolpalub, x, y, ky, kx, hits, arrkolpalub, k = 0 } = this;
			...
			player.squadron[shipname] = {arrkolpalub, hits, x, y, ky, kx};
			// если корабль создан для игрока, выводим его на экран
			if (player === human) {
				Ships.showShip(human, shipname, x, y, ky);
	...
	class Placement {
		...
		static FRAME_COORDS = getCoordinates(myArea_field);
		...
		setObserver() {
            ...
            myArea_field.addEventListener('doubletap', this.rotationShip.bind(this));
			flagHandPos = true;
		}

		onMouseDown(EO) {
			// если нажата не левая кнопка мыши или игра уже запущена
			if (EO.which != 1 || flagStart) return;
		...
		onMouseMove(e) {
			...
			if (left >= Placement.FRAME_COORDS.left - 14 && right <= Placement.FRAME_COORDS.right + 14 && top >= Placement.FRAME_COORDS.top - 14 && bottom <= Placement.FRAME_COORDS.bottom + 14) {
				...
				const result = human.checkLocationShip(obj, this.kolpalub);
				if (!result) {
		...
		rotationShip(e) {
			...
			if (human.squadron[name].kolpalub == 1) return;
			...
			const kolpalub = human.squadron[name].arrkolpalub.length;
			...
			const result = human.checkLocationShip(obj, kolpalub);
			if(!result) {
				obj.ky = (obj.ky == 0) ? 1 : 0;
				obj.kx = (obj.kx == 0) ? 1 : 0;
			}
		...
		getCoordsCloneInMatrix({left, right, top, bottom} = coords) {
			...
			let ft = (computedTop < 0) ? 0 : (computedBottom > Field.sizeArea) ? Field.sizeArea - Field.sizeShip : computedTop;
			let fl = (computedLeft < 0) ? 0 : (computedRight > Field.sizeArea) ? Field.sizeArea - Field.sizeShip * this.kolpalub : computedLeft;

			obj.top = Math.round(ft / Field.sizeShip) * Field.sizeShip;
			obj.left = Math.round(fl / Field.sizeShip) * Field.sizeShip;
			// переводим значение в координатах матрицы
			obj.x = obj.top / Field.sizeShip;
			obj.y = obj.left / Field.sizeShip;
		...
		removeShipFromSquadron(el) {
			...
			const arr = human.squadron[name].arrkolpalub;
	...
	class Controller {
		...
		static getCoordsIcon = el => {
			const x = el.style.top.slice(0, -2) / Field.sizeShip;
			const y = el.style.left.slice(0, -2) / Field.sizeShip;
			return [x, y];
		}
		...
		init() {
			...
			if (!flagController) {
				//выстрел игрока
				pcArea.addEventListener('click', this.makeShot.bind(this));
				// устанавливаем маркер на заведомо пустую клетку
				pcArea.addEventListener('contextmenu', this.setUselessCell.bind(this));
				flagController = true;
			}
		...
		setCoordsAroundHit(x, y, coords) {
			let {firstHit, ky, kx} = this.tempShip;
			...
			// второе попадание, т.к. оба коэффициента равны 0
				...
				this.tempShip.ky = (Math.abs(firstHit[0] - x) == 1) ? 1 : 0;
				this.tempShip.kx = (Math.abs(firstHit[1] - y) == 1) ? 1 : 0;
			}
		...
		isShipSunk() {
			...
			let obj = Object.values(human.squadron)
				.reduce((a, b) => a.arrkolpalub.length > b.arrkolpalub.length ? a : b);
			// определяем, есть ли ещё корабли, с кол-вом палуб больше, чем попаданий
			if (this.tempShip.hits >= obj.arrkolpalub.length || this.coordsAroundHit.length == 0) {
		...
		transformCoordsInMatrix(e, self) {
			const x = Math.trunc((e.pageY - self.fieldTop) / Field.sizeShip);
			const y = Math.trunc((e.pageX - self.fieldLeft) / Field.sizeShip);
			return [x, y];
		}
		...
		markUselessCellAroundShip(){
			...
			const {hits, ky, kx, x0, y0} = this.tempShip;
			let coords;
		...
		showIcons(opponent, [x, y], iconClass) {
			...
			function fn() {
				...
				span.style.cssText = `left:${y * Field.sizeShip}px; top:${x * Field.sizeShip}px;`;
		...
		hit(x, y) {
			...
			for (let name in this.opponent.squadron) {
				...
				for (let value of dataShip.arrkolpalub) {
					...
					dataShip.hits++;
					if (dataShip.hits < dataShip.arrkolpalub.length) break outerloop;
			...
			if (Object.keys(this.opponent.squadron).length == 0) {
				if (this.opponent === human) {
					...
					for (let name in computer.squadron) {
						...
						Ships.showShip(computer, name, dataShip.x, dataShip.y, dataShip.ky );
					}
				} else {
	...
	const human = new Field(myArea_field);
	...
	buttonNewGame.addEventListener('click', function(e) {
		...
		pcArea.parentElement.hidden = true;
